...
def stumycourse(request):
    pass
    return render(request, 'login/stumycourse.html')
...
def tea1(request):
    if request.method == "POST":
        ...
        Course.objects.create(courseName=courseName, courseCredit=courseCredit, courseTeacher_id=courseTeacher,
                              courseClassNum=classnum, courseClassType=classtype, courseMaxNum=peoplenum,
                              courseOverTime=overtime, courseStartTime=starttime)
...
# 删除课程
def deleClass(request):
    print(request.GET)
    id = request.GET.get("userid")
    print("ddd"+str(id))
    Course.objects.filter(id=id).delete()
    course_student.objects.filter(student_id=id).delete()
    teacher_id = request.session['user_id']
    course_inform = Course.objects.filter(courseTeacher_id=teacher_id)
    context = {
        "course_inform": course_inform,
